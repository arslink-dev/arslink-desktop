...
static void WriteSidecar(const wchar_t *path, EXCEPTION_POINTERS *pException, bool dumpWritten) {
    ...
    AppendA(file, "ArsLink crash report\r\n===================\r\n");
...
static void WriteCrashArtifacts(EXCEPTION_POINTERS *pException) {
    ...
    wsprintfW(dumpPath, L"%s\\ArsLink_%s_%s_%04d%02d%02d-%02d%02d%02d.dmp",
              g_dumpPathTemplate, L"" NKR_VERSION, THRONE_ARCH,
              st.wYear, st.wMonth, st.wDay, st.wHour, st.wMinute, st.wSecond);
    wsprintfW(sidecarPath, L"%s\\ArsLink_%s_%s_%04d%02d%02d-%02d%02d%02d.txt",
              g_dumpPathTemplate, L"" NKR_VERSION, THRONE_ARCH,
              st.wYear, st.wMonth, st.wDay, st.wHour, st.wMinute, st.wSecond);
    ...
    wsprintfW(msg,
              ...
              addrHex,
              g_dumpPathTemplate);
    MessageBoxW(nullptr, msg, L"ArsLink crashed", MB_OK | MB_ICONERROR | MB_SETFOREGROUND);
}
...
void Windows_ConfigureWER() {
    ...
    const bool ok = RegisterWerApp(exeName, g_dumpPathTemplate) &&
                    RegisterWerApp(L"ArsLinkCore.exe", g_dumpPathTemplate);
    if (ok) {
        LOG_INFO("WER LocalDumps registered for ArsLink.exe and ArsLinkCore.exe");
    } else {

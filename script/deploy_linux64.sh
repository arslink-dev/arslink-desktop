...
#### copy binary ####
cp $GITHUB_WORKSPACE/build/ArsLink $DEST
...
./linuxdeploy-$ARCH.AppImage --appdir $DEST --executable $DEST/ArsLink --plugin qt
...
# fix lib rpath
cd $DEST
patchelf --set-rpath '$ORIGIN/usr/lib' ./ArsLink

# handle debug info
objcopy --only-keep-debug $DEST/ArsLink $DEST/ArsLink.debug
strip --strip-debug --strip-unneeded $DEST/ArsLink
objcopy --add-gnu-debuglink=$DEST/ArsLink.debug $DEST/ArsLink